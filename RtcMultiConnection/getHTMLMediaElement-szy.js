...
function getHTMLMediaElement(mediaElement, config) {
    ...
    mediaControls.className = 'vctt-r';
    mediaElementContainer.appendChild(mediaControls);

    //≈‰÷√ ”∆µ
    if (buttons.has('mute-video')) {
        var muteVideo = document.createElement('i');
        muteVideo.className = 'iconfont ' + (config.toggle.has('mute-video') ? 'icon-videoImg' : 'icon-videoImg');
        mediaControls.appendChild(muteVideo);

        muteVideo.onclick = function () {
            if (muteVideo.className.indexOf('icon-videoImg icon-videoImgNo') != -1) {
    ...
    if (buttons.has('mute-desk')) {
        var mutedesk = document.createElement('i');
        mutedesk.className = 'iconfont ' + (config.toggle.has('mute-desk') ? 'icon-pm' : 'icon-pm');
        mediaControls.appendChild(mutedesk);
        mutedesk.onclick = function () {
            if (mutedesk.className.indexOf('icon-pm') != -1) {
                ...
                mutedesk.className = mutedesk.className.replace('icon-pm', 'icon-sxt');

            }
            else {//«–ªª…„œÒÕ∑

                mutedesk.className = mutedesk.className.replace('icon-sxt', 'icon-pm');
...
function getMainElement(mediaElement, config) {
    ...
    var buttons = config.buttons || ['mute-audio', 'mute-video', 'mute-desk', 'stop', 'full-screen'];
    buttons.has = function (element) {
        return buttons.indexOf(element) !== -1;
    };
