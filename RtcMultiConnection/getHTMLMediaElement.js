...
function getHTMLMediaElement(mediaElement, config) {
    ...
    mediaElementContainer.setAttribute('data-streamid', config.streamid);
    ...
    mediaElementContainer.style.width = config.width - 14 +"px";
    mediaElementContainer.style.height = config.width * 3 / 4 + 4 +"px";
    ...
    if (buttons.has('full-screen')) {
        ...
        zoom.onclick = function () {
            if (zoom.className.indexOf('zoom-out') != -1) {
                ...
                launchFullscreen(mediaElementContainer.children[2].children[1]);
            }
        };
